// Entry point of the mail-dispatcher service.
//
// Unlike the other four it exposes no public API (01 section 2): it is an event consumer.
// It still runs the operational HTTP server -- /livez, /readyz, /metrics -- because the
// orchestrator and the probes need it, and because a consumer that cannot report its own
// health is a consumer nobody notices dying. That port is not routed by Traefik.
//
...
import (
	...
	"github.com/nicodanke/dizen-v2-backend/services/mail-dispatcher/internal/config"
	"github.com/nicodanke/dizen-v2-backend/services/mail-dispatcher/internal/db/migrations"
	"github.com/nicodanke/dizen-v2-backend/services/mail-dispatcher/internal/repository"
)

const serviceName = "mail-dispatcher"
